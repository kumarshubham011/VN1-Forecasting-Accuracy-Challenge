...
def data_competition_evaluation(phase="Phase 0", name=""):

    # Submission should be loaded from a .csv file.
    submission = pd.read_csv(name)
    assert all(col in submission.columns for col in [
    ...
    objective = pd.read_csv(
        f"{phase} - Sales.csv").set_index(["Client", "Warehouse", "Product"])
...
def data_competition_dummy_submission_phase1():
    ...
    data = pd.read_csv(
        "Phase 0 - Sales.csv").set_index(["Client", "Warehouse", "Product"])
    ...
    submission = pd.DataFrame(
        index=data.index,
        columns=pd.date_range(start=data.columns.max(),
                              periods=14, freq="W-MON", inclusive="neither"),
    ...
    submission = pd.DataFrame(
        ...
        columns=pd.date_range(start=data.columns.max(),
                              periods=14, freq="W-MON", inclusive="neither"),
        data=np.repeat(np.nanmean(
            data.values[:, -12:], axis=1).reshape(-1, 1), 13, axis=1)
    )
    submission.to_csv(name)
    data_competition_evaluation(phase="Phase 1", name=name)
